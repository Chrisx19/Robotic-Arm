...
//  g_Joints_disable_Joy = true;
  ...
  uint16_t joint_adc_min[5] = {107, 268, 36, 32, 34};
  uint16_t joint_adc_max[5] = {177, 302, 129, 127, 133};

  uint16_t elbow =      map(elbow_cmd,      -90, 90, 36, 140);          //min 35(servo write cmd)~ -90 degree ; max 140~ 90 degree/////////////MID ~ 88
  uint16_t wrist =      map(wrist_cmd,       90, -90, 34, 143);          //min 34~ -90 degree ; max 143~ 90 degree
  uint16_t wrist_roll = map(wrist_roll_cmd, -90, 90, 35, 145);          //min 35~ -90 degree ; max 135~ 90 degree
...
//*********************************Motor Commands***********************************
//  stepper_position_1(waist_val_raw, waist_cmd, 1000 ,3);
//  stepper_position_2(shoulder_val_raw, shoulder_cmd, 500 ,25);
  elbow_joint3.slowmove(elbow, 10);
  wrist_joint4.slowmove(wrist, 10);
  wrist_roll_joint5.slowmove(wrist_roll, 10);
//*********************************Motor Commands***********************************
...
void stepper_position_2(int sensor, int target, int velocity,int threshold)
{
  ...
  if (sensor > (target - threshold) && sensor < (target + threshold))
  {
    ...
    shoulder_joint2.setSpeed(g_decel);
    delay(30);
  }
  else
  {
