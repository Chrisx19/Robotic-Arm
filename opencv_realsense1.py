...
#initialize position values before calculation
x_d=0.0
y_d=0.0
x_d_p=0.0
y_d_p=0.0
...
while(1):
    ...
    #show fps of camera feed on top left of video capture frame
    cv2.putText(webcam, fps, (19, 25), font, 0.8, (100, 255, 0), 1, cv2.LINE_AA)

    ########### function for finding green blocks ###########
        ...
        for pic, contour in enumerate(contours):
            if len(contours)>0:
                ...
                if area>500: 
                    ...
                    ###########create bounding rectangle around face of block###########
                    ...
                    #### display angle and color near detected block ####
                    cv2.putText(webcam, 
                                'Angle:'+str(angle),
                    ...
                    #### draw line from base (point of reference) to center of block ####
                    webcam=cv2.circle(webcam, (contour[0][0][0], contour[0][0][1]), 5, (255,255,255), -1) #dot marking the base
                    ...
                    x_d= (((2*y+h)/2)-68) * 0.06		#calculate distance
                    y_d= (((2*x+w)/2)-260) * 0.075
                    ######################################################################

                    ##### Detect Depth of block ######
                    ...
                    #### Display text for for y_d and x_d on detected block ####
                    cv2.putText(webcam, 
    ...
    ########### function for finding blue blocks ###########
    ...
    ########### function for finding red blocks ###########
