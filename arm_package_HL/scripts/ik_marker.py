...
def processFeedback(feedback):
    ...
    if feedback.event_type == InteractiveMarkerFeedback.MENU_SELECT:
        if feedback.menu_entry_id==1:
            ...
            listener.waitForTransform('/end_effector_link','/base_link',rospy.Time(), rospy.Duration(1.0))
            (trans,rot)=listener.lookupTransform('base_link','end_effector_link',rospy.Time())
            print (trans,rot)
...
if __name__=="__main__":
	...
	print (listener.frameExists('end_effector_link'))
	(trans,rot)=listener.lookupTransform('base_link','end_effector_link',rospy.Time())
	print (trans,rot)
	...
	menu_handler.insert( "Move to end_effector_link", callback=processFeedback )
